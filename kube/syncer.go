...
// Renderer renders a set of [Object]s to be synchronized into the Kubernetes
// API via [Syncer] using SSA.
...
// ownership and garbage collecting objects no longer rendered.
	...
	for _, obj := range toSync {
		if err := s.Ctl.Apply(ctx, obj, client.ForceOwnership); err != nil {
			...
			if meta.IsNoMatchError(err) {
				...
				DebounceError(logger, err, "WARNING no registered value for resource type", "gvk", gvk.String(), "key", AsKey(obj))
				continue
			}
			return nil, err
		}
...
// DeleteAll deletes all objects under the syncer's purview. It returns a
// boolean indicating whether any objects remained alive (i.e. had a non-nil
// DeletionTimestamp) after the deletion attempt.
	...
	for _, t := range s.Renderer.Types() {
		...
		if err != nil {
			...
			if meta.IsNoMatchError(err) {
				// the WARNING messages here get logged constantly and are fairly static containing the resource type itself
				// so we can just use the global debouncer which debounces by error string
				DebounceError(logger, err, "WARNING no registered value for resource type", "gvk", gvk.String())
				continue
			}
			return nil, err
		}

...
var (
	// ErrInvalidPodFQDN is returned when a pod FQDN cannot be parsed.
...
func (p *PodDialer) DialContext(_ context.Context, network string, address string) (net.Conn, error) {
	...
	if err != nil {
		return nil, errors.Join(fmt.Errorf("creating error stream: %w", err), conn.Close())
	}

	headers.Set(corev1.StreamType, corev1.StreamTypeData)
	dataStream, err := conn.CreateStream(headers)
	if err != nil {
		return nil, errors.Join(fmt.Errorf("creating data stream: %w", err), conn.Close())
	}

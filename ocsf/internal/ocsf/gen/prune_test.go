...
func TestPruneForIceberg_CycleLenTwo(t *testing.T) {
	...
	// The forward edge survives structurally; the back edge is demoted to a
	// string (present, not dropped) which breaks the cycle.
	require.Contains(t, s.Objects["alpha"].Attributes, "to_beta")
	require.Contains(t, s.Objects["beta"].Attributes, "to_alpha")
	require.Equal(t, "string_t", s.Objects["beta"].Attributes["to_alpha"].Type)
	require.Empty(t, s.Objects["beta"].Attributes["to_alpha"].ObjectType)
}
...
func TestPruneForIceberg_ConstraintScrub(t *testing.T) {
	...
	s := pruneSchema(
		...
		map[string]*schema.Object{
			"wrap": {Name: "wrap", Attributes: map[string]*schema.Attribute{}}, // empty → R4 drops root.w
		},
	)
...
func TestPruneForIceberg_EmptyMessageCascade(t *testing.T) {
	s := pruneSchema(
		...
		map[string]*schema.Object{
			...
			"bag": {Name: "bag", Attributes: map[string]*schema.Attribute{}}, // empty source object → R4 drops edges to it
		},
	)
...
func TestPruneSidecarFile(t *testing.T) {
	...
	require.Equal(t,
		"# Code generated by ocsf-protogen --iceberg-compat. DO NOT EDIT.\n"+
			...
			"# R1/R2/R3 demote the field to a string holding its JSON (data kept,\n"+
			"# structure not queryable); R4 drops it. See prune.go for the rules.\n"+

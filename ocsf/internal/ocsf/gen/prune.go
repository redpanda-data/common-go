...
func PruneSidecarFile(version string, prunes []PrunedField) (GeneratedFile, error) {
	...
	sb.WriteString("# R1-value-to-string demotes a google.protobuf.Value field to string;\n")
	sb.WriteString("# R2/R3/R4 drop the field. See prune.go for the rules.\n")
	...
	for _, cls := range classes {
		for _, name := range sortedKeys(cls.Attributes) {
			if isValueAttr(cls.Attributes[name]) {
				demoteValueToString(cls.Attributes[name])
	...
	for _, objName := range reachableObjectNames(s, classes) {
		...
		for _, name := range sortedKeys(obj.Attributes) {
			if isValueAttr(obj.Attributes[name]) {
				demoteValueToString(obj.Attributes[name])
...
// demoteValueToString retypes a google.protobuf.Value attribute to the OCSF
...
func demoteValueToString(attr *schema.Attribute) {
